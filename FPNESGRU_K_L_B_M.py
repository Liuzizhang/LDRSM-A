...
from .FastBiGRU import FastMinimalGRUBidirectional  # 确保FastMinimalGRUBidirectional已正确导入
class TimeAttentionGRU(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers=1):
        super().__init__()
        self.gru = FastMinimalGRUBidirectional(
            input_size=input_dim,
            hidden_size=hidden_dim // 2,  # 确保这里用hidden_dim//2
            batch_first=True
        )
        # 调整注意力层输入维度为hidden_dim（双向输出）
...
class FPN_ESGRU_K_L_B_M(nn.Module):
    def __init__(self, _, in_channel, out_channel, base_dim=32):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(base_dim*2, base_dim),  # 输入维度保持base_dim*2
            nn.GELU(),
