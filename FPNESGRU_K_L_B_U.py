...
class FPN_ESGRU_K_L_B_U(nn.Module):
...
class LightConvGRU(nn.Module):
    ...
    def forward(self, x):
        ...
        h = torch.zeros(batch_size, self.hidden_dim, seq_len).to(x.device)
        
        # 卷积GRU计算
        for t in range(seq_len):
            combined = torch.cat([x[:,t,:], h[:,:,t]], dim=1).unsqueeze(-1)
            
            z = self.conv_z(combined).squeeze(-1)
            r = self.conv_r(combined).squeeze(-1)
            h_hat = self.conv_h(torch.cat([x[:,t,:], r*h[:,:,t]], dim=1).unsqueeze(-1)).squeeze(-1)
            
            h = (1 - z) * h + z * h_hat
        
        # 轻量注意力
        attn_weights = self.attention(h.permute(0,2,1))  # [B, T, 1]
        return torch.sum(h * attn_weights.permute(0,2,1), dim=2)